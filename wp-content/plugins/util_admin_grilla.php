...
function upload_logo($file){
	...
	$thum= get_theme_root() . '/televisionvtr/img/foto/'.$img_nombre;
	$nails = $img_nombre;
	$_POST[$file]=$nails;
  ...
  <div class="wrap">
    ...
    foreach ($r as $canal) {
        echo '<tr valign="top" class="alternate author-other status-publish " id="chn-'.$canal->codigo.'">
            <td>'.$i.'</td>
            <td><img src="/wp-content/themes/televisionvtr/img/foto/' .  $canal->imagen . '" width="48" /></td>
            <td>' . $canal->codigo . '</td> 
    ...
    if($_GET["editar"]){
        ...
        foreach ($r1 as $r1data){
            foreach ($r1data as $key => $value) {
                ...
                if ($key == "imagen" || $key == 'imagentrans' || $key == "imagen40x40"){$typeInput = "file"; $img = '<img src="/wp-content/themes/televisionvtr/img/foto/' .  $value . '" width="20" />';}else{$img = "";}
